...
- 订阅状态：展示 seats/到期/续费状态，并支持取消自动续费
...
### [1.0.15] - 2026-02-02
#### 修复
- 管理后台“团队管理”页面：点击“添加团队”或“编辑团队”时自动滚动到表单并聚焦输入框，解决页面滚动到底部后看不到表单的问题。
#### 变更
- 忽略本地开发产生的 `/.gemini-clipboard/` 目录（`.gitignore`）。

### [1.0.14] - 2026-01-27
#### 新增
- 管理后台成员弹窗支持批量降级 Owner 以及单个降级操作。
- 团队订阅状态展示（自动续费/已取消续费、到期时间、周期、席位数）。
- 管理后台新增“取消自动续费”操作。

### [1.0.13] - 2026-01-24
#### 修复
- 修复“上游邀请”取消失败（HTTP 422）：对齐 ChatGPT 官网删除 Pending invite 的请求 payload（`email_address`）。
